...
parser.add_argument('--data', metavar='DIR',
                    #help='path to training dataset',default='/home/roit/datasets/Sintel/training')
                    help='path to training dataset', default='/home/roit/datasets/kitti_scene_flow/training/')

#parser.add_argument('--dataset', metavar='DATASET', default='mpi_sintel_final',
parser.add_argument('--dataset', metavar='DATASET', default='KITTI_occ',
                    choices=dataset_names,
                    help='dataset type : ' +
                    ' | '.join(dataset_names))
parser.add_argument('--pretrained', dest='pretrained', default='/home/roit/models/flownet/trained_self/flownets_best.pth.tar',
...
parser.add_argument('--print-freq', '-p', default=100, type=int,#很多训练都是每个epoch打印一次结果在终端 ,这里就每10次forwardprint一次， 最大不大过len(trainloader / batchsize)
                    metavar='N', help='print frequency')
parser.add_argument('-e', '--evaluate', dest='evaluate', action='store_true',default=False,#if true 就evaluate一次当前模型效果，然后解树
...
#Create a mutually exclusive group. argparse will make sure that only one of the arguments in the mutually exclusive
# group was present on the command line

group = parser.add_mutually_exclusive_group()#
group.add_argument('-s', '--split-file', default=None, type=str,
                   help='test-val split file')
group.add_argument('--split-value', default=0.8, type=float,# 80 percent to train and the rest to val
...
#tensorboardX
    train_writer = SummaryWriter(os.path.join(save_path,'train'))#'KITTI_occ/05-29-11:36/flownets,adam,300epochs,epochSize1000,b8,lr0.0001'
